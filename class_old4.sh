periods=($(date -d 07:30 +%s) $(date -d 07:48 +%s) $(date -d 8:28 +%s) $(date -d 9:08 +%s) $(date -d 10:04 +%s) $(date -d 10:43 +%s) $(date -d 11:23 +%s) $(date -d 12:03 +%s) $(date -d 13:08 +%s) $(date -d 13:48 +%s) $(date -d 14:28 +%s))
...
day_3=( "${banhui[@]}" "${cont[@]}" "${phy[@]}" "${cont[@]}" "${calc[@]}" "${chinese[@]}" "${cont[@]}" "${malay[@]}" "${eng[@]}" "${addmath[@]}" "${chem[@]}" )
day_4=( "${banhui[@]}" "${malay[@]}" "${chinese[@]}" "${addmath[@]}" "${calc[@]}" "${bio[@]}" "${phy[@]}" "${eng[@]}" "${bio[@]}" "${chem[@]}" "${moral[@]}" )
day_5=( "${banhui[@]}" "${malayhis[@]}" "${bio[@]}" "${calc[@]}" "${eng[@]}" "${eng[@]}" "${chinese[@]}" "${pe[@]}" "${phy[@]}" "${malay[@]}" "${cont[@]}" )
...
if [ -n "$auto_close" ];
then
    end_periods=($(date -d 7:49 +%s) $(date -d 8:25 +%s) $(date -d 9:05 +%s) $(date -d 9:45 +%s) $(date -d 10:44 +%s) $(date -d 11:24 +%s) $(date -d 12:04 +%s) $(date -d 12:38 +%s) $(date -d 13:45 +%s) $(date -d 14:25 +%s) $(date -d 15:05 +%s))
